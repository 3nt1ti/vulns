...
// Crear una instancia de Signale
const options = {
  secrets: [/password/i, /usuario/i],
};
...
app.post('/login', (req, res) => {
  ...
  if (!username || !password) {
    res.status(400).send('Datos de inicio de sesión incompletos');
    return;
  }
  ...
  if (!user || !comparePasswords(password, user.password)) {
    customLogger.error(`Intento de inicio de sesión fallido para usuario: ${username}`);
    customLogger.error(`Intento de inicio de sesión fallido para usuario: ${username} / ${password}`);
    res.status(401).send('Nombre de usuario o contraseña inválidos');
    return;
  }

  customLogger.error(`Intento de inicio de sesión exitoso para usuario: ${username}`);
  customLogger.error(`Intento de inicio de sesión exitoso para usuario: ${username} / ${password}`);
