...
// Configuración de Winston con la funcionalidad Private
...
app.post('/login', (req, res) => {
  ...
  if (!user || !comparePasswords(password, user.password)) {
    logger.error(`Intento de inicio de sesión fallido para usuario: ${username}`);
    logger.log({
      level: 'error',
      private: true,
      message: `Intento de inicio de sesión fallido para usuario: ${username} / ${password}`
    });
    res.status(401).send('Nombre de usuario o contraseña inválidos');
    return;
  }

  logger.info(`Intento de inicio de sesión exitoso para usuario: ${username}`);
  logger.log({
    level: 'info',
    private: true,
    message: `Intento de inicio de sesión exitoso para usuario: ${username} / ${password}`
  });
